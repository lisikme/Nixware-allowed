...
def extract_build_number(version_str):
    ...
    if numbers:
        try:
            return int(numbers[-1])  # Берем последнее число (обычно это билд)
        except ValueError:
            return 0
    return 0
...
def is_server_updated(server_version, cs2_version):
    ...
    if server_version == "unknown" or cs2_version == "unknown":
        return True  # Если не можем определить, считаем что обновлен
...
def get_connect_status(server_version, cs2_version, server_status):
    ...
    if server_version == "unknown" or cs2_version == "unknown":
        return "online"  # Если не можем определить версии, считаем онлайн
...
async def get_server_info(server_ip, server_port):
    ...
    try:
        info = await a2s.ainfo(address) # type: ignore
        ...
        status = "Online"
        try:
            players = await a2s.aplayers(address) # type: ignore
            processed_players = []
            ...
            logger.warning(f"Ошибка при получении списка игроков {server_ip}:{server_port}: {e}")
            processed_players = []
...
async def process_servers_from_file():
    ...
    servers = []
    try:
        # Убираем кавычки и пробелы, разделяем по запятым
        ...
        return  # Выходим без очистки файла
    ...
    try:
        ...
        # Очищаем исходный файл только при успешном выполнении
...
def start_file_monitor():
    ...
    def monitor_loop():
        while monitor_running:
            try:
                # Используем asyncio для запуска асинхронной функции в отдельном потоке
                import asyncio
...
async def get_multiple_servers_raw(servers_list: str):
    """Получить информацию о нескольких серверах в одном ответе"""
    ...
    try:
        ...
        log_error(error_msg)
        raise HTTPException(status_code=400, detail=error_msg)
    console.print(f"🔍 Обработка {len(servers)} серверов (RAW)...", style="yellow")
    servers_dict = {}
    for server in servers:
        try:
            info = await get_server_info(server["ip"], server["port"])
...
async def get_specific_server(server_ip: str, server_port: int):
    """Получить информацию о конкретном сервере"""
